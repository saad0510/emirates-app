class AppAssets {
  static const images = "assets/images";
  static const dest = "$images/destinations";

  static const logo = "$images/logo extra small.png";
  static const onboard1 = "$images/passport.png";
  static const onboard2 = "$images/suitcase.png";
  static const onboard3 = "$images/plane cartoon.png";
  static const googleIcon = "$images/google icon.png";
  static const facebookIcon = "$images/facebook icon.new.png";
  static const planeTrace = "$images/plane in air.png";
  static const planeView = "$images/plane inner view.png";
  static const planeWindows = "$images/plane windows.png";
  static const seat = "$images/seat shape.png";
  static const creditCard = "$images/credit card.png";
  static const successMark = "$images/success.png";
  static const worldMap = "$images/world map.png";

  static const mainCover = "$dest/man with bike cover.png";
  static const rockInWater = "$dest/mountain in water.png";
  static const bali = "$dest/bali.png";
  static const hanalei = "$dest/hanalei beach.png";
  static const hapuna = "$dest/hapuna beach.png";
  static const hawai = "$dest/hawai.png";
  static const makena = "$dest/makena beach.png";
  static const maldives = "$dest/maldives.png";
  static const waima = "$dest/waima beach.png";
  static const pantai = "$dest/pantai.png";
  static const islandView = "$dest/island view.jpeg";
}
